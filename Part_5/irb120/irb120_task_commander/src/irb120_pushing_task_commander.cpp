//irb120_pushing_task_commander.cpp
...
int main(int argc, char** argv) {
	...
	//the goal position to move the gear
    double xGoal = 0.30;
	double yGoal = 0.30;
	
	//the offset for hand to move down so it does not move on top of gear
	double offset = 0.065;
	double xOffset = 0;
	double yOffset = 0;
	...
	//calculate which side of gear hand should move down on
	if(g_perceived_object_pose.pose.position.x > xGoal+0.01){
		xOffset = offset;
		yOffset = 0;
	}
	else if(g_perceived_object_pose.pose.position.x < xGoal-0.01){
		xOffset = -offset;
		yOffset = 0;
	}
	else if(g_perceived_object_pose.pose.position.y > yGoal+0.01){
		xOffset = 0;
		yOffset = offset;
	}
	else if(g_perceived_object_pose.pose.position.y < yGoal-0.01){
    ...
    flange_origin << g_perceived_object_pose.pose.position.x + xOffset, g_perceived_object_pose.pose.position.y + yOffset, 0.5; //specify coordinates for the desired flange position (origin) with respect to the robot's base frame
    ...
    flange_origin <<g_perceived_object_pose.pose.position.x + xOffset, g_perceived_object_pose.pose.position.y + yOffset, 0.005; 
    ...
    ros::Duration(1.5).sleep(); //dwell here to  observe contact
    //xxxxxxxxxxxxxxxxxx


	//determine which direction to push
	if(g_perceived_object_pose.pose.position.x > xGoal+0.01){
		flange_origin << xGoal + xOffset, g_perceived_object_pose.pose.position.y, 0.005;
	}
	else if(g_perceived_object_pose.pose.position.x < xGoal-0.01){
    	flange_origin << xGoal + xOffset, g_perceived_object_pose.pose.position.y, 0.005;
	}
	else if(g_perceived_object_pose.pose.position.y > yGoal+0.01){
    	flange_origin << g_perceived_object_pose.pose.position.x, yGoal + yOffset, 0.005;
	}
	else if(g_perceived_object_pose.pose.position.y < yGoal-0.01){
    	flange_origin << g_perceived_object_pose.pose.position.x, yGoal + yOffset, 0.005;
	}
    ...
    flange_origin << g_perceived_object_pose.pose.position.x + xOffset, g_perceived_object_pose.pose.position.y + yOffset, 0.5;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
